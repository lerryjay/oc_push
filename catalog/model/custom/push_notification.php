<?php
  ...
  class ModelCustomPushNotification extends Model {
    private $webpushServer = 'http://localhost/push-app/api/webpush/';
    
    public function getPushKeys(){
      $this->load->model('setting/setting');
      $config = $this->model_setting_setting->getSetting('custom_push');
      if(!$config){
        $key = $this->do_curl($this->webpushServer.'getkey',[],['api-key: 852e0b-868862-1510e2-412cdd-9bf302']);
     ...
     public function addNotificationSubcriber($userId,$token,$accounttype = 'buyer',$platform = 'mobile'){
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "push_subscribers SET customer_id = '".(int)$userId."', token = '".$token. "', usertype = '".$accounttype."'");
